...
def mostrar_laboratorio():
    ...
    if ejecutar:
        try:
            # 1. CÁLCULOS DE ESTADO (Idem anterior)
            if par == "P y T":
                t_in = v2
            ...
            # ... (se pueden agregar los otros casos aquí)

            # --- GRÁFICOS CON RANGOS DE SEGURIDAD ---
            ...
            # Datos de saturación para límites
            sf_crit = PropsSI('S', 'T', t_crit+273.15-1, 'Q', 0, sustancia)/1000
            sg_min = PropsSI('S', 'T', t_min+273.15+1, 'Q', 1, sustancia)/1000
            ...
            if "T-s" in tipo_grafico:
                ...
                # ISOBARA SEGURA: Limitamos el rango de entropía para evitar el error "Out of Range"
                s_min_iso = min(sf) * 0.5
                s_max_iso = max(sg) * 1.2
                s_range = np.linspace(s_min_iso, s_max_iso, 50)
                ...
                for s in s_range:
                    try:
                        ...
                        if temp < 1000: # Filtro de seguridad
                            t_iso.append(temp)
                ...
                fig.update_layout(yaxis=dict(range=[t_min-10, t_crit+100]), xaxis_title="s [kJ/kgK]", yaxis_title="T [°C]")
                ...
                v_range = np.logspace(np.log10(min(vf)*0.2), np.log10(max(vg)*5), 50)
                p_iso = []
                v_iso_final = []
                for v in v_range:
                    try:
