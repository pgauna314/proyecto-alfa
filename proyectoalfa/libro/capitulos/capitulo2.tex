...
En este capítulo, definiremos la Termodinámica como disiciplina y abordaremos los conceptos fundamentales de \textbf{sistema}, \textbf{balance de materia} y \textbf{balance de energía}.

En lugar de iniciar con definiciones académicas abstractas, presentaremos el caso de la Central Térmica Río Turbio, a través de la cual  analizaremos cómo estos principios se aplican en la generación de potencia eléctrica, para finalmente formalizar los conceptos clave.



\section{Introducción: Central Térmica}
\label{sect:intro_centrales_termicas}
...
Podemos resumir el proceso en términos generales de la siguiente forma: la energía química del combustible se transforma en calor que vaporiza el agua; este a su vez se transforma en energía cinética (movimiento de la turbina) y finalmente ese movimiento se transforma en energía eléctrica. Así es como se define formalmente la Termodinámica: 

\begin{definicion}{Sistema Abierto}
La Termodinámica es la parte de la Física que estudia la energía y sus transformaciones.
\end{definicion}
...
\subsection{La Central Térmica Río Turbio}
La Central Térmica de Río Turbio está emplazada en la provincia de Santa Cruz y genera una potencia eléctrica de 240 MW. Desde el punto de vista termodinámico, su funcionamiento puede explicarse con las cuatro operaciones básicas descriptas en la sección \ref{sect:intro_centrales_termicas}. El fluido intermediario es agua, y el combustible que emplea es carbón, dado que la zona cuenta con un yacimiento del mineral.



\section{Nuestras primeras herramientas de análisis}
Para estudiar cómo las energías se transforman unas en otras en una planta de generación de potencia, necesitaremos un conjunto de herramientas fundamentales. En primer lugar, debemos aprender a delimitar nuestra región de estudio mediante la definición de un \textbf{volumen de control}, donde se establecen fronteras imaginarias. Una vez fijados estos límites, es necesario saber contar energía y materia, partiendo de la base de que la cantidad de una depende intrínsecamente de la cantidad de la otra. Este proceso de contabilidad, regido por las leyes de conservación, nos permitirá entender no solo cuánto fluido circula por las instalaciones, sino cómo ese flujo transporta y transforma la energía en cada etapa del ciclo térmico.


\subsection{El volumen de control o sistema abierto}

Supongamos que queremos analizar termodinámicamente el agua que circula por una cañería dentro de la caldera. Para ello, trazaremos una línea imaginaria que abarque esa cañería, como se muestra en la Figura \ref{fig:EleccionSistema} (recuadro rojo). Esta elección nos permite definir tres regiones:
...
\subsubsection{Lo que tenés que saber sobre los sistemas abiertos}
...
\subsubsection{Estado transitorio: fase de llenado}
...
\subsubsection{Estado estacionario: operación en régimen permanente}
...
\subsubsection{Necesidad de una descripción en términos de flujo}
...
\subsubsection{Planteo general del balance de materia}
...
\subsubsection{Formalismo matemático del balance de materia}
